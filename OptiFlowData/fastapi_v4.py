...
def optimize_inflow(water_level, capacity, outflow, start_datetime, max_flow=250, smoothness_weight=1):
    """
    배수지 유입량 최적화 (유입량 변화 스무딩)
    ...
    Args:
        ...
        model_path: 모델 경로 (사용 안 함 - 목적 함수 단순화)
        max_flow: 최대 유입량
        smoothness_weight: 유입량 변화 스무딩 가중치 (조절 가능)
    ...
    # 유입량 변화량 (L1 norm) 최소화 -> 스무딩 효과
    for h in range(23):  # 마지막 시간은 다음 시간과의 차이를 계산할 수 없음
    ...
    prob += level_at_9am + deviation_9am >= max_safe_level
    prob += level_at_9am - deviation_9am <= max_safe_level
...
# def optimize_inflow(water_level, capacity, outflow, start_datetime, max_flow=250):
#   min_safe_level = capacity * 0.55
#   max_safe_level = capacity * 0.9
#   hourly_rates = get_hourly_rates(start_datetime)
