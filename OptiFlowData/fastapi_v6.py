...
def optimize_inflow(water_level, capacity, outflow, start_datetime, min_flow=18, max_flow=292, smoothness_weight=10):
    """
    ...
    """
    min_safe_level = capacity * 0.70
    max_safe_level = capacity * 0.90
...
async def predict_data_lstm(request: Request, input_data: InputData):
  ...
  opti_inflow = optimize_inflow(input_data.waterLevel, capacity_global[input_data.name], outflow_lstm, start_datetime, max_flow=380)
...
async def predict_data_xgb(request: Request, input_data: InputData):
  ...
  opti_inflow = optimize_inflow(input_data.waterLevel, capacity_global[input_data.name], outflow_xgb, start_datetime, max_flow=380)
